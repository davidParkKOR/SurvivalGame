...
public class ItemEffect
{
    ...
    [Tooltip("HP, SP, DP, HUNGRY, THIRSTY 만 가능합니다.")]
    public string[] part; //어느 부위를 회복시킬지
    public int[] num; //수치 (고기일경우 굶주림 10, 썩은고기일 경우 굶주림 10, 체력 -10) 이런식
                ...
                if (itemEffects[x].itemName == _item.itemName)
                {
                    for (int y = 0; y < itemEffects[x].part.Length; y++)
                    {
                        switch (itemEffects[x].part[y])
                        {
                            ...
                            default:
                                Debug.Log("잘못된 Status 부위 HP, SP, DP, HUNGRY, THIRSTY 만 가능합니다.");
                                break;
                        }
